      ...
      if (pba->has_scf == _TRUE_&& pba->scf_has_perturbations == _TRUE_) {
        if (pba->scf_evolve_like_axionCAMB == _FALSE_){
          ppv->y[ppv->index_pt_phi_scf] =
            ppw->pv->y[ppw->pv->index_pt_phi_scf]; // LG delta phi 

          ppv->y[ppv->index_pt_phi_prime_scf] =
            ppw->pv->y[ppw->pv->index_pt_phi_prime_scf]; // LG delta phi prime
            // printf("Vector init delta phi prime \n");
        }
...
// PH approx. function starts //LG
void PH_values(       struct background * pba,
                      struct perturbations * ppt,
                      struct perturbations_workspace * ppw,
                      double k,
                      double y1,
  ...
  {
  // Initialise PH and local variables
  double H = 0.;
  double Hprime = 0.;
  double factor_PH = 0.;
  double phi_c = 0.;
  ...
  H = sqrt((ppw->pvecback[pba->index_bg_rho_tot])-pba->K/a/a); 

  Hprime = - (3./2.) * ((ppw->pvecback[pba->index_bg_rho_tot]) + 
          (ppw->pvecback[pba->index_bg_p_tot])) * a + pba->K/a;

  factor_PH = 6*pow(H,2)/(9*pow(H,4) - 4*(4*pow(H,2)*pow(mass,2) + pow(Hprime,2)/pow(a,2)));

  phi_c = phi_scf;

  phi_prime_c = factor_PH*a*mass*(4*H*mass*phi_scf + (3*pow(H,2)*phiprime_scf/
                (mass*a)) +(2*Hprime*phiprime_scf/(mass*pow(a,2))));

  phi_s = (phiprime_scf - phi_prime_c)/(a*mass);

  phi_prime_s = factor_PH*a*mass*(3*pow(H,2)*phi_scf - 2*Hprime*phi_scf/a + 4*H*phiprime_scf/a);
  ...
  term = (mass*a/k) * (sqrt(1 + pow((k/(a*mass)), 2)) - 1);
  cs2_scf = pow(term, 2) + 1.25 * pow((H/mass),2);

  w_scf_f = 1.5*pow((H/mass),2);

  ca2_scf = w_scf_f - (Hprime)/(a*pow(mass, 2)*(1 + w_scf_f));

  theta_scf = (1./3.)*(pow(k,2)*mass/(2*a)) * (delta_phi_c*(phi_s + phi_prime_c/(a*mass)) + delta_phi_s*(-phi_c + phi_prime_s/(a*mass))) 
  ...
  PH_variables[1] = delta_p_scf;
  PH_variables[2] = cs2_scf;
  ...
  PH_variables[10] = phi_prime_s;
  PH_variables[11] = delta_phi_c;
    ...
    */
      ...
      if (pba->has_scf == _TRUE_ && pba->scf_has_perturbations == _TRUE_) {
        ...
        */
        ...
        if(pba->scf_evolve_like_axionCAMB == _FALSE_) { //CHANGE HERE? //LG
          ...
          // LG delta phi prime
          // printf("Initial conditions delta phi prime ");
    ...
    if (ppt->gauge == newtonian) {
      ...
      */
      ...
      if (pba->has_scf == _TRUE_ ) { // CHANGES HERE?
      ...
      if (pba->has_scf == _TRUE_&& pba->scf_has_perturbations == _TRUE_) {
        ...
        alpha_prime = 0.0; // LG ZERO???
          ...
          ppw->pv->y[ppw->pv->index_pt_phi_scf] += alpha*ppw->pvecback[pba->index_bg_phi_prime_scf]; // LG delta phi
          ...
          (-2.*a_prime_over_a*alpha*ppw->pvecback[pba->index_bg_phi_prime_scf]
           ...
           +ppw->pvecback[pba->index_bg_phi_prime_scf]*alpha_prime); // LG delta phi prime
  ...
  if (_scalars_) {
    ...
    if (ppt->has_source_delta_scf == _TRUE_) {
      ...
      // printf("Sources delta phi prime ");
        ...
        _set_source_(ppt->index_tp_delta_phi_scf) = y[ppw->pv->index_pt_phi_scf]; // LG delta phi
        _set_source_(ppt->index_tp_delta_phi_over_phi_scf) = y[ppw->pv->index_pt_phi_scf]/ppw->pvecback[pba->index_bg_phi_scf];
        _set_source_(ppt->index_tp_delta_phi_prime_scf) = y[ppw->pv->index_pt_phi_prime_scf]; // LG delta phi prime
  ...
  if (_scalars_) {
    ...
    if (pba->has_scf == _TRUE_&& pba->scf_has_perturbations == _TRUE_){
      ...
      //If we are following KG:
          ...
          delta_phi_scf = y[ppw->pv->index_pt_phi_scf]; // LG delta phi
          delta_phi_over_phi_scf = y[ppw->pv->index_pt_phi_scf]/ppw->pvecback[pba->index_bg_phi_scf];
          delta_phi_prime_scf = y[ppw->pv->index_pt_phi_prime_scf]; // LG delta phi prime
      ...
      //If we are behaving as a fluid, mimicking fld:
        ...
        delta_phi_scf = y[ppw->pv->index_pt_phi_scf];//0; // LG delta phi 
        delta_phi_over_phi_scf = y[ppw->pv->index_pt_phi_scf]/ppw->pvecback[pba->index_bg_phi_scf];//0;
        delta_phi_prime_scf = y[ppw->pv->index_pt_phi_prime_scf];//0; // LG delta phi prime  //LG delta phi printed as zero is this the issue for the outputted values?? - YES
      }
...
int perturbations_derivs(double tau,
  ...
  double f_norm=0;
      ...
      if(pba->scf_evolve_like_axionCAMB == _TRUE_){
        ...
        if(pba->scf_evolve_as_fluid == _FALSE_){
          ...
          // if(pvecback[pba->index_bg_Omega_scf] <= pba->threshold_scf_fluid_m_over_H && pvecback[pba->index_bg_a] > pba->a_c){
            ...
            // printf("here fld a %e\n", pvecback[pba->index_bg_a]);
            // pba->scf_fluid_eq = 1;
            // printf("Derivs switch occurred, delta_rho: %e, scale_factor: %e, theta: %e, Flag: %d, k: %e \n", delta_rho_scf, a, y[ppw->pv->index_pt_theta_scf], ppt->scf_kg_eq[index_md][index_k], k); //LG 16/09/24
            // exit(0);
          }
          else {
            // printf("KG: a %e k %e\n", pvecback[pba->index_bg_a],k);
   ...
   //CO 22.01.18 scf_evolve_as_fluid == _TRUE_ means user wants to use fluid equation. scf_parameters[0]>3H means this mode is in the regime where fluid eqs apply.
    ...
    // if(k < 1.1e-3 && k > 1.0e-3){
    // printf("DERIVS FLAG %d, scale factor: %e, k: %e \n", ppt->scf_kg_eq[index_md][index_k], ppw->pvecback[pba->index_bg_a], k); //LG
    // }
...
//            if(pba->has_cdm==_TRUE_ && ppt->DMDE_interaction > 0)dy[pv->index_pt_theta_fld] -= ppt->DMDE_interaction*a/ppw->pvecback[pba->index_bg_rho_cdm]*(ppw->pvecback[pba->index_bg_rho_cdm]/((1+w_fld)*ppw->pvecback[pba->index_bg_rho_fld]))*(y[pv->index_pt_theta_fld]-y[pv->index_pt_theta_cdm]); /* cdm velocity */
    ...
    if (pba->has_scf == _TRUE_ && pba->scf_has_perturbations == _TRUE_) {
      if (ppt->scf_kg_eq[index_md][index_k] == 1) {
        ...
        /** - ----> field value */
        dy[pv->index_pt_phi_scf] = y[pv->index_pt_phi_prime_scf]; // LG delta phi prime
        // printf("Derivs delta phi prime ");
