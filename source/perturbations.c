...
void PH_delta(
  ...
  // return delta_rho_scf/ppw->pvecback[pba->index_bg_rho_scf]; // NEED TO ALSO RETURN DELTA_P?????

  // y[ppw->pv->index_pt_delta_scf] = delta_rho_scf/ppw->pvecback[pba->index_bg_rho_scf];
  aux_deltas[0] = delta_rho_scf;
  aux_deltas[1] = delta_p_scf; // NEED TO DEFINE ARRAY LOCALLY BEFORE PASSING INTO THIS FUNCTION
...
int perturbations_total_stress_energy(
  ...
  // PH and local variables
  double aux_deltas[2]; // 0=delta_rho, 1=delta_p // CAN INCLUDE MORE AND MAKE LARGER
  double H = 0.;
  double Hprime = 0.;
  double factor_PH = 0.;
  double phi_c = 0.;
            ...
            (1./a2*ppw->pvecback[pba->index_bg_phi_prime_scf]*y[ppw->pv->index_pt_phi_prime_scf]
             ...
             if(pba->scf_evolve_as_fluid == _TRUE_){
              ...
              if(pba->scf_evolve_as_fluid_PH == _TRUE_)
              {
                ...
                PH_delta(pba, ppt, ppw, k, aux_deltas);
                y[ppw->pv->index_pt_delta_scf] = aux_deltas[0]/ppw->pvecback[pba->index_bg_rho_scf];
